...
### `cleanupRoom(roomId)`
Видаляє порожню кімнату (без гравців).
...
4. Видаляє з `clientToRoom`, `clientRole`
5. Видаляє з `room.playerColors` (але зберігає в `originalPlayerOrder`)
6. Оновлює список кімнат
7. Видаляє порожню кімнату (якщо `room.players.length === 0`)
...
- Кімната автоматично видаляється коли всі гравці відключаються
